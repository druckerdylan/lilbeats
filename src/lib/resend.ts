...
/*
  TODO: the fallback is on `lilbeats.com`, a domain the project does not own,
  so it could never authenticate — Resend rejects a from-address on an
  unverified domain, and the send fails outright. DKIM and SPF are already
  published for `lilbeatsofficial.com`, so this should become a mailbox
  there (e.g. orders@) once Dylan decides which one. Until then set
  `RESEND_FROM_EMAIL` in the environment; this constant is the failure case,
